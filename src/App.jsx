...
function Landing({ onStart, onGuides }) {
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      const maxScroll = sceneRef.current.scrollHeight - vh;
      if (maxScroll > 0) setProgress(Math.min(window.scrollY / maxScroll, 1));
    };
  ...
  const doodleH = 285;
  const doodleY = (-doodleH + 20) + ((vh - doodleH - 30) - (-doodleH + 20)) * progress;
  const swayX = Math.sin(progress * Math.PI * 4) * 25;
  const centerX = (typeof window !== "undefined" ? window.innerWidth : 400) / 2 - 100;
  const doodleScale = 1 - progress * 0.15;
  const doodleRot = Math.sin(progress * Math.PI * 3) * 3;
  const phase = progress < 0.28 ? 0 : progress < 0.55 ? 1 : progress < 0.78 ? 2 : 3;

  const cloudVis = (i) => {
    const showAt = 0.02 + i * 0.07;
    const vis = progress > showAt && progress < showAt + 0.22;
    const drift = (progress - showAt) * 45;
    return { opacity: vis ? 1 : 0, transform: `translateX(${i % 2 === 0 ? drift : -drift}px)`, transition: "opacity .6s ease, transform .6s ease" };
  };
  const Cloud = ({ i, style, path, w, vb }) => <div style={{ position: "absolute", ...style, width: w, pointerEvents: "none", ...cloudVis(i) }}>
    <svg viewBox={vb} style={{ width: "100%" }}><path d={path} fill="none" stroke="rgba(255,255,255,.18)" strokeWidth="1.5" strokeLinecap="round" /></svg>
  </div>;

  const phaseStyle = (i) => ({
    position: "absolute", width: "100%", textAlign: "center", padding: "0 32px", zIndex: 5,
    pointerEvents: (i === 3 && phase === 3) ? "auto" : phase === i ? "auto" : "none",
    opacity: (i === 3 ? phase >= 3 : phase === i) ? 1 : 0,
    transform: (i === 3 ? phase >= 3 : phase === i) ? "translateY(-50%)" : "translateY(-40%)",
  ...
  return <div style={{ color: "#fff", fontFamily: BF, position: "relative", overflow: "hidden" }}>
    ...
    <style>{`@keyframes sway { 0%,100% { transform: rotate(-2deg); } 50% { transform: rotate(2deg); } } @keyframes ctaGlow { 0%,100% { opacity: .4; transform: translate(-50%,-50%) scale(1); } 50% { opacity: .6; transform: translate(-50%,-50%) scale(1.1); } } @keyframes scrollPulse { 0%,100% { opacity:.4 } 50% { opacity:.8 } }`}</style>
    ...
    <div ref={sceneRef} style={{ position: "relative", height: "300vh" }}>
      <div style={{ position: "sticky", top: 0, height: "100vh", width: "100%", overflow: "hidden", zIndex: 1 }}>
        ...
        {/* Clouds — 12 total */}
        <Cloud i={0} style={{ top: "8%", left: "4%" }} w={130} vb="0 0 130 40" path="M8 30 Q30 12 55 25 Q72 10 95 22 Q112 14 125 28" />
        <Cloud i={1} style={{ top: "14%", right: "5%" }} w={105} vb="0 0 105 30" path="M5 22 Q25 8 50 18 Q68 5 90 20 Q98 14 103 22" />
        <Cloud i={2} style={{ top: "22%", left: "18%" }} w={75} vb="0 0 75 24" path="M5 18 Q18 6 35 15 Q50 4 72 17" />
        <Cloud i={3} style={{ top: "28%", right: "12%" }} w={115} vb="0 0 115 32" path="M6 24 Q28 8 50 20 Q70 6 90 18 Q102 10 112 22" />
        <Cloud i={4} style={{ top: "36%", left: "6%" }} w={90} vb="0 0 90 26" path="M5 20 Q22 6 42 16 Q58 4 78 14 Q86 9 88 18" />
        <Cloud i={5} style={{ top: "42%", right: "8%" }} w={65} vb="0 0 65 20" path="M4 15 Q18 5 32 12 Q46 3 62 14" />
        <Cloud i={6} style={{ top: "50%", left: "12%" }} w={100} vb="0 0 100 28" path="M5 22 Q22 8 45 18 Q62 5 82 16 Q92 10 97 20" />
        <Cloud i={7} style={{ top: "56%", right: "16%" }} w={55} vb="0 0 55 18" path="M4 13 Q15 4 28 11 Q40 3 52 12" />
        <Cloud i={8} style={{ top: "64%", left: "22%" }} w={80} vb="0 0 80 24" path="M5 18 Q20 6 38 15 Q55 4 75 16" />
        <Cloud i={9} style={{ top: "70%", right: "20%" }} w={48} vb="0 0 48 16" path="M4 12 Q14 4 24 10 Q34 3 44 11" />
        <Cloud i={10} style={{ top: "78%", left: "8%" }} w={60} vb="0 0 60 18" path="M4 14 Q16 4 30 12 Q42 3 56 13" />
        <Cloud i={11} style={{ top: "84%", right: "10%" }} w={72} vb="0 0 72 20" path="M5 15 Q18 5 34 13 Q48 4 68 14" />

        {/* Doodle figure */}
        <div style={{ position: "absolute", transform: `translate(${centerX + swayX}px, ${doodleY}px)`, willChange: "transform", zIndex: 10 }}>
          <div style={{ transform: `rotate(${doodleRot}deg) scale(${doodleScale})` }}>
        ...
        {/* Ground line */}
        <div style={{ position: "absolute", bottom: 26, left: 0, right: 0, height: 2, background: "linear-gradient(90deg, transparent 10%, rgba(159,225,203,.2) 30%, rgba(159,225,203,.3) 50%, rgba(159,225,203,.2) 70%, transparent 90%)", opacity: progress > 0.85 ? 1 : 0, transition: "opacity .4s" }} />
        ...
        {/* Phase 4: CTAs */}
        <div style={{ ...phaseStyle(3), transform: phase >= 3 ? "translateY(-60%)" : "translateY(-50%)" }}>
          {!showTerms ? <>
        ...
        {/* Scroll hint */}
        <div style={{ position: "absolute", bottom: 28, left: "50%", transform: "translateX(-50%)", textAlign: "center", opacity: progress > 0.1 ? 0 : 1, transition: "opacity .3s", zIndex: 20, pointerEvents: "none" }}>
    ...
    {/* ─── FEATURES SECTION ─── */}
    <div style={{ position: "relative", zIndex: 1, padding: "80px 28px 100px", background: "linear-gradient(180deg, #0A2540 0%, #0A2540 100%)" }}>
